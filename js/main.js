(() => {
  ...
  const applyProfile = () => {
    ...
    document.querySelector('meta[name="description"]').content =
      `${config.name}의 웹 개발 포트폴리오. ${config.tagline.replace(/\n/g, " ")}`;
  ...
  // URL 앵커는 콘텐츠가 렌더링되기 전에 브라우저가 먼저 이동하므로,
  // 비동기로 채워지는 프로젝트 목록 등 렌더링이 끝난 뒤 위치를 다시 보정합니다.
  const alignInitialHash = () => {
    const hash = window.location.hash;
    // 단순 ID 형태의 앵커만 처리합니다. (예: #projects)
    ...
    const align = () => {
      if (cancelled) return;
      // 초기 로드 보정은 애니메이션 없이 즉시 이동합니다.
      target.scrollIntoView({ behavior: "auto" });
    };
    // 사용자가 직접 스크롤하면 자동 보정을 멈춰 조작을 방해하지 않습니다.
    const cancel = () => {
      cancelled = true;
    };
    ...
    // 초기 동기 렌더링 직후, 그리고 이미지·폰트 로딩 후 위치를 맞춥니다.
    window.requestAnimationFrame(align);
    window.addEventListener("load", () => window.requestAnimationFrame(align));

    // 프로젝트 목록처럼 비동기로 채워지는 콘텐츠까지 반영해 다시 보정합니다.
    const projectsRegion = document.querySelector("#projects-region");
    if (projectsRegion && "MutationObserver" in window) {
      const observer = new MutationObserver(() =>
        window.requestAnimationFrame(align),
      );
      observer.observe(projectsRegion, { childList: true, subtree: true });
      // 일정 시간이 지나면 관찰을 종료해 이후 상호작용에 영향을 주지 않습니다.
